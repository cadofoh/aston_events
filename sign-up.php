...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //Check if email fielld is empty
    if (empty(trim($_POST["email"]))) {
        $emailErr = "You Forgot to Enter Your Email!";
    } else {
        $email = sanitize(trim($_POST["email"]));

        // Validation to accept only aston emails
        $ending = 'aston.ac.uk';
    ...
    if (!empty($_POST["password"]) && ($_POST["password"] == $_POST["confirm_password"])) {
        ...
        $confirm_password_err = "Please Ensure Both Passwords Are The Same";
    } else {
    ...
    if (
        empty($firstErr) && empty($lastErr) && empty($username_err) && empty($emailErr) && empty($phoneErr)
